...
class ManagePanel
{
    ...
    function createUser($name_panel, $usernameC, array $config_data)
    {
        $Output = [];
        global $connect;
        ...
        $panel_data = select("marzban_panel", "*", "name_panel", $name_panel, "select");
        $expire = $config_data['expire'];
        $data_limit = $config_data['data_limit'];
        if ($panel_data['type'] == "marzban") {
            //create user
            $ConnectToPanel = adduser($usernameC, $expire, $data_limit, $panel_data['name_panel']);
            ...
            if (isset($data_Output['detail']) && $data_Output['detail']) {
                ...
                if (!preg_match('/^(https?:\/\/)?([a-zA-Z0-9-]+\.)+[a-zA-Z]{2,}(:\d+)?((\/[^\s\/]+)+)?$/', $data_Output['subscription_url'])) {
                    $data_Output['subscription_url'] = $panel_data['url_panel'] . "/" . ltrim($data_Output['subscription_url'], "/");
                }
        ...
        } elseif ($panel_data['type'] == "x-ui_single") {
            $subId = bin2hex(random_bytes(8));
            $Expireac = $expire * 1000;
            $data_Output = addClient($panel_data['name_panel'], $usernameC, $Expireac, $data_limit, generateUUID(), "", $subId);
            if (!$data_Output['success']) {
                ...
                $domain = explode(":", $panel_data['linksubx']);
